  ...
  if(!empty($_SESSION['carro'])){
    ...
    foreach($_SESSION['carro'] as $codess){
        ...
        while($fila = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
          $suma += $fila['Precio']*$codess['cantidad'];
        }
        mysqli_free_result($consulta);
    }
    if(consultasSQL::InsertSQL("venta", "Fecha, NIT, Descuento, TotalPagar, Estado, NumeroDeposito, Adjunto", "'".date('d-m-Y')."','$Cedclien','0','$suma','$StatusV','$NumDepo','$comprobanteF'")){
      ...
      foreach($_SESSION['carro'] as $carro){
      		$preP=ejecutarSQL::consultar("SELECT Precio FROM producto WHERE CodigoProd='".$carro['producto']."'");
      		$filaP=mysqli_fetch_array($preP, MYSQLI_ASSOC);
          	consultasSQL::InsertSQL("detalle", "NumPedido, CodigoProd, CantidadProductos, PrecioProd", "'$Numpedido', '".$carro['producto']."', '".$carro['cantidad']."', '".$filaP['Precio']."'");
